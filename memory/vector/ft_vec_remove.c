...
/*   Updated: 2023/07/15 22:53:10 by mdekker       ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	*ft_vec_remove(t_vector *v, size_t i)
{
	...
	if (i > v->size)
		return (NULL);
	new = ft_calloc(v->size - v->type_size, 1);
	if (!new)
		return (NULL);
	ft_memcpy(new, v->data, i);
	ft_memcpy(new + i, v->data + i + v->type_size, v->size - i - v->type_size);
	free(v->data);
	v->data = new;
	v->size -= v->type_size;
	return (v->data);
